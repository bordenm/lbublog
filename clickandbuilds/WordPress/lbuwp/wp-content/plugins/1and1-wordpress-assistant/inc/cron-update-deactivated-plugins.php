...
class One_And_One_Cron_Update_Deactivated_Plugins {
	...
	public function __construct() {
		...
		include_once( One_And_One_Assistant::get_inc_dir_path().'stats-logger.php' );
	...
	public function log( $text ) {
		if ( ! file_exists( One_And_One_Assistant::get_plugin_dir_path().'/log' ) ) {
			mkdir( One_And_One_Assistant::get_plugin_dir_path().'/log' );
		}

		file_put_contents( One_And_One_Assistant::get_plugin_dir_path().'/log/cron-update-deactivated-plugins.log', date( '[ d.m.Y H:i:s ] ' ).$text."\n", FILE_APPEND );
	}
	...
	public function logRemote( $extra_args = array() ) {
		...
		$status = One_And_One_Assistant_Stats_Logger::logRemote( $args );
